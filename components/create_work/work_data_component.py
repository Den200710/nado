...
class WorkDataComponent(BaseComponent):
    def __init__(self, page: Page):
        ...
        self._input_fields = {
            ...
            'order_name': page.locator("//textarea[@id='order_name']"),
            'plot': page.locator("(//div[contains(@class,'form-control')]//textarea)[3]"),
        }
        # Словарь с переключателями
        self._toggles = {
            'nightly_work': page.locator("label:has-text('Работа проводится в вечернее/ночное время')"),
            'temperature_measurement': page.locator("label:has-text('Требуется замер температуры или ГВС')"),
            'holiday_work': page.locator("label:has-text('Работа проводится в выходные/праздничные дни')"),
            'planned_repair': page.locator("label:has-text('Плановые ремонтные работы')"),
        }
